...
- **One loud thing per screen.** Only `vencido` and `vence_em_breve` are painted with the status tint (`AppStatus.isLoud`), and it goes on the icon well and the state line, never on a whole row or a whole card. On Início the loud thing is the attention strip: the verdict as its head, the items under it, all between two hairlines and never inside a red box.
...
- **Início answers "now" and "next", and nothing else.** The attention strip lists what is late or close from every domain (three at most), with "Ver todos (N)" opening `/avisos` (`AlertsScreen`, the full `GET /alerts`) when the dashboard could not show all of it. "Próximos cuidados" is `dashboard.upcoming`: what comes next among the things that are fine, each with how far it is (`upcomingSummary`: "Faltam 4.200 km ou 30 dias") and a bar **only** when `planProgress` could form a real fraction from figures the server sent — never an estimated one. Costs and the last fill are **not** on Início: they head Histórico, where "what did this car cost" is the reason to open the tab. The odometer caption says how old the reading is past six weeks, because every distance-based due date is measured from it.
