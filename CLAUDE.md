...
- **Lists are rows, and rows live in a group.** `AppListRow` (icon, name, one line of state) inside `AppGroup` (`lib/shared/widgets/app_group.dart`): the label stays **outside** and quiet, the rows go **inside** one filled, rounded surface with hairlines between them. It is the grouped list every phone already uses for settings, and it says the three things a bare stack cannot — these rows are one thing, this thing ends here, the label is a name and not a row. `appGroupGap` is the distance between two groups; use it rather than picking a number.
...
- **Sections label, they do not shout.** `AppSectionHeader` is `labelLarge` in `onSurfaceVariant`, deliberately *quieter* than the `bodyLarge` item names under it. It used to be `titleMedium` and competed with its own content. `AppGroup` builds it for you; reach for the header alone only outside a group.
- **`AppSurface` is the primitive for a block that is not a list** — a tinted status band, a single reading. Its default variant is `none`, and `AppCard` is the grouped variant of it, kept for detail screens where one object and its facts is a real group. A card around a single sentence is the pattern that started all this — use a label and the sentence.
...
- **One loud thing per screen.** Only `vencido` and `vence_em_breve` are painted with the status tint, and the one place that does it on Início is the verdict row at the foot of the vehicle panel. `AppPriorityBanner` used to be that element and was deleted with the panel that replaced it: two ways to say "here is the verdict" is how two screens start disagreeing about what a verdict looks like.
...
- **There is no global add button, but Início names two or three.** The centred "+" over the navigation bar opened a sheet of seven things from screens where none of them was the obvious move; that is still gone and stays gone. What Início has instead is a row of *named* tiles — Abastecer, Manutenção, Atualizar km — each going straight to its own form, on the one screen where all of them are plausible. A tile that cannot apply is absent, not disabled: a car that does not refuel gets two. `AddRecordSheet` is Histórico's own action, and it is the one place a choice among record types is the right question.
...
- **Início answers "now" and "next".** "Precisa de atenção" lists what is late or close from every domain, with "Ver todos (N)" opening `/avisos` (`AlertsScreen`, the full `GET /alerts`) when the dashboard could not show all of it — "Ver todos" used to open the maintenance tab, which lists maintenance only. "Em seguida" is `dashboard.upcoming`: what comes next among the things that are fine. The cost block is hidden while nothing was spent, and the odometer caption says how old the reading is past six weeks, because every distance-based due date is measured from it.
