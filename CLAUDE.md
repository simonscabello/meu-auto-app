...
**The design system** lives in [`docs/DESIGN-SYSTEM.md`](./docs/DESIGN-SYSTEM.md) and is the reference for every screen since the redesign of 24/09/2026: a night-time instrument cluster — navy page with one soft light, surfaces one step up with a hairline edge, one electric-blue accent, red only for what is late, amber for what is close. Dark is the default theme (`ThemeModeStore` falls back to `dark`); light is the same identity in daylight and stays in Perfil. Tokens are `lib/core/theme` (`AppColors`, `AppTones` for what a `ColorScheme` cannot name, `AppTypography` with Inter for text and Rajdhani for instrument figures, `AppStatus`/`statusColors`, `AppSpacing`, `AppRadius`, `AppMotion`) and **no colour, radius or spacing literal lives outside that folder**. The **visual identity of the native assets** (icon, splash) lives in [`docs/IDENTIDADE-VISUAL.md`](./docs/IDENTIDADE-VISUAL.md); the four PNGs in `assets/icon/` are processed art and a palette change goes through `tool/recolor_icons.dart`, never an editor. Regenerating native resources is two commands, recorded there — and the launcher-icon generator corrupts one line of the iOS project file every time, also recorded there.
...
- **Lists are rows, and rows live in a group.** `AppListRow` (icon in an `AppIconWell`, name, one line of state) inside `AppGroup` (`lib/shared/widgets/app_group.dart`): the label stays **outside** and quiet, the rows go **inside** one filled surface with a hairline edge and hairlines between them. It is the grouped list every phone already uses for settings, and it says the three things a bare stack cannot — these rows are one thing, this thing ends here, the label is a name and not a row. `appGroupGap` is the distance between two groups; use it rather than picking a number.
...
- **Sections label, they do not shout.** `AppSectionHeader` defaults to `label` — `labelLarge` in `onSurfaceVariant`, deliberately *quieter* than the item names under it. `AppSectionEmphasis.title` exists for the one or two sections that are the point of a screen ("Próximos cuidados" on Início) and its action is a link with a chevron, never a button. `AppGroup` builds the header for you; reach for it alone only outside a group.
- **`AppSurface` is the primitive for a block that is not a list** — a tinted status band, a single reading, an upcoming item on Início. `grouped` is one step off the page with a hairline; `raised` is two steps with a stronger edge and light along the top, and is reserved for a surface that is an action (`AppQuickAction`, the odometer pencil). `AppCard` is the grouped variant, kept for detail screens where one object and its facts is a real group. A card around a single sentence is the pattern that started all this — use a label and the sentence.
- **Four tabs, one per job: Início, Manutenção, Documentos, Histórico.** Perfil is the account and lives behind the account button (`ProfileButton`) on every tab's app bar — it used to hold a tab while Histórico, one of the product's three promises, was the last row of the maintenance list. The route for Manutenção is still `/cuidados`, so old deep links keep working. Every tab other than Início shows which car it is about under its title as soon as there are two (`VehicleContextTitle`), and the title everywhere uses `Vehicle.shortName` ("Toyota Prius", not "Toyota PRIUS 1.8 16V 5p Aut. (H…").
- **One loud thing per screen.** Only `vencido` and `vence_em_breve` are painted with the status tint (`AppStatus.isLoud`), and it goes on the icon well and the state line, never on a whole row or a whole card. On Início the loud thing is the attention strip: the verdict alone, between two hairlines and never inside a red box, opening what it counts.
...
- **There is no global add button, and Início names exactly two.** The centred "+" over the navigation bar opened a sheet of seven things from screens where none of them was the obvious move; that is still gone and stays gone. What Início has instead is two `AppQuickAction` tiles of exactly equal weight — Abastecer, Registrar manutenção — each going straight to its own form. The mileage is not a third tile: the pencil beside the reading is the one prominent way to update it, and two prominent ways to do one thing is how a screen stops saying what matters. A tile that cannot apply is absent, not disabled: a car that does not refuel gets one, full width. `AddRecordSheet` is Histórico's own action, and it is the one place a choice among record types is the right question.
- **The verdict has four states, and "we do not know" is one of them.** Late, close, *unknown*, fine (`verdictOf`). The phrase names what it counts: "2 itens vencidos", with "Mais 1 vence em breve" on the quiet line — it used to say "2 itens precisam de atenção" above a group of that name listing three. It used to have three, and a brand-new car — or one whose owner answered "não sei" to every history question — got a green "Tudo em dia" while the app knew nothing about it. Unknown is neutral, never tinted: "Nada vencido até agora" with "N itens sem data da última vez" under it, read from `alerts.unknown_history` (falling back to `needs_baseline` on an older server). The earlier rule — keep setup prompts off Início so a new car does not look broken — still holds for the *tint*; it went too far when it also hid the fact. One quiet row, "Conte o que já foi feito no carro", opens the history questions while any are unasked, and disappears when they are answered. `profilePromptOf` still covers the two gaps that stop the app working — no fuel type, no plan at all.
- **Início answers "now" and "next", and nothing else.** The attention strip is the verdict and nothing under it: "1 item vencido" opens that item, "3 itens vencidos" opens `/avisos` (`AlertsScreen`, the full `GET /alerts`). It used to list the items under the count as well, and the owner tapped "1 item vencido" to land on a page that repeated "Calibrar os pneus" — the same fact three times on two screens. Do not put the rows back on Início. "Próximos cuidados" is `dashboard.upcoming`: what comes next among the things that are fine, each with how far it is (`upcomingSummary`: "Faltam 4.200 km ou 30 dias") and a bar **only** when `planProgress` could form a real fraction from figures the server sent — never an estimated one. Costs and the last fill are **not** on Início: they head Histórico, where "what did this car cost" is the reason to open the tab. The odometer caption says how old the reading is past six weeks, because every distance-based due date is measured from it.
...
**Bundled fonts, no font package.** Inter (text) and Rajdhani (instrument figures, through `AppTypography.instrument` only) ship in `assets/fonts/` under the OFL, subsetted to Latin. Widget tests load them, so a text width in a test is a real width.
