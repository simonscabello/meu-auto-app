...
- **There is no global add button, but Início names two or three.** The centred "+" over the navigation bar opened a sheet of seven things from screens where none of them was the obvious move; that is still gone and stays gone. What Início has instead is a row of *named* tiles — Abastecer, Manutenção, Km atual — each going straight to its own form, on the one screen where all of them are plausible. A tile that cannot apply is absent, not disabled: a car that does not refuel gets two. `AddRecordSheet` is Histórico's own action, and it is the one place a choice among record types is the right question.
- **Início reports on the car; it does not chase the owner for setup.** `alerts.needs_baseline` is deliberately not rendered there, and `statusOf` has no `semBaseline` branch. A new vehicle has one of those per suggested plan, none of them urgent, and putting a dozen of them where the verdict goes made a healthy car look broken. The prompt lives in Cuidados, in the group named "Falta informar", where someone has already come to deal with upkeep. `profilePromptOf` survives on Início for exactly two gaps that stop the app working — no fuel type, no plan at all — and counts nothing that can only be answered elsewhere.
