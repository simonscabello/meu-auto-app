...
- **Four tabs, one per job: Início, Manutenção, Documentos, Histórico.** Tabs have no app bar: they open with `VehicleTabHeader` (`lib/features/vehicle/presentation/vehicle_context_title.dart`) — a large title on the page gutter, the car on the line under it in the accent ("Prius · QAF5G33", always, which opens the vehicle switcher), the tab's own "+" and the account avatar (`ProfileButton` on `AppAvatar`: the owner's photo, or their initial). Perfil is not a tab. The route for Manutenção is still `/cuidados`, so old deep links keep working.
...
- Dependencies are exactly: `flutter_localizations`, `flutter_riverpod`, `go_router`, `dio`, `flutter_secure_storage`, `shared_preferences`, `intl`, `uuid`, `url_launcher`, `image_picker`. Adding another is a decision, not a detail. The redesign added none: the fonts are assets, the gradient is two `DecoratedBox`es, and the pressed-state scale is an `AnimatedScale`. `image_picker` (the flutter.dev plugin) came with the profile photo on 25/09/2026, because camera and gallery access are platform plumbing with no honest hand-written alternative; it also shrinks the photo to 1024px before upload. There is deliberately no crop package and no `cached_network_image`: the avatar is `BoxFit.cover` in a circle, and `Image.network` is enough for one small picture whose signed URL changes on every `/me` anyway.
...
**The APK ships as a GitHub Release, and the app says when a newer one exists.** A tag `v1.2.0` that matches the pubspec's `version:` runs `.github/workflows/release-apk.yml`: analyze and test (with the backend checked out beside it, so the contract test is not skipped), a release build signed with the upload key from four repository secrets — it fails on purpose without them, because a debug-signed APK cannot be installed over the one on the phone — and a Release carrying `meu-auto-<v>.apk` plus a fixed-name `meu-auto.apk`. The server's public `GET /v1/app-version` returns `APP_LATEST_VERSION` and `APP_APK_URL` (Railway); `lib/features/update` compares that with `AppConfig.appVersion` and puts "Atualizar o Meu Auto" as the first row of Início, **in the header's slot** — it is about the app, not one of the five things Início answers about the car, and the header draws in the loading and error states too, which is when an old build most needs it. The tap hands the link to the browser; the app installs nothing itself. Three things are deliberate: **`APP_VERSION` is a define only the workflow passes**, so a build from this machine is never asked to replace itself, and no package-info plugin was added to learn a version the workflow already knows; **every failure is silence**, since the notice is an invitation and never an error; and **publishing and announcing are separate steps** — the Release can exist for days before `APP_LATEST_VERSION` asks anyone to install it. The checklist is `docs/RODANDO.md` §5.2. `.github/workflows/ci.yml` runs analyze and test on every push to `main`.

**Publishing a version — what "sobe uma versão" means, in this order.** The first one published this way was 1.1.0 (versionCode 2), on 26/09/2026, and the owner's phone runs it.

0. If the version depends on a backend change, the backend goes first: a push to its `main` deploys it on Railway. Wait until the new route answers before tagging the app.
1. Bump `version:` in `pubspec.yaml`, both parts — `1.1.0+2` → `1.1.1+3`. The build number is the Android versionCode and only ever grows; the three numbers grow too, because the update check compares them before the build.
2. Commit, push `main`, then tag that commit `vX.Y.Z` and push the tag. The tag must equal the pubspec version without the build, or the workflow stops at its first check.
3. Wait for the Release, about seven minutes. `https://github.com/simonscabello/meu-auto-app/releases/latest/download/meu-auto.apk` answers 404 until it exists and 302 once it does — poll that page, not the GitHub API, which allows 60 anonymous requests an hour and `gh` is not logged in on this machine.
4. Download that APK and check its signer: `apksigner verify --print-certs` must show the certificate SHA-256 recorded in `docs/RODANDO.md` §5.2. Any other certificate will not install over the phones' copy.
5. Set `APP_LATEST_VERSION` to the new version on Railway (project MEU AUTO, service `meu-auto-backend`) and confirm `GET /v1/app-version` returns it. Only then does Início say "Atualizar o Meu Auto".

**Never hand the owner an APK built on this machine as an update.** There is no `android/key.properties` here, so a local release build is signed with the debug key, and Android refuses to install it over a copy that came from a Release. Local builds are for the emulator and for testing; phones get the Release.
